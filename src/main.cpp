...
// Copyright (c) 2015 The Heirloom Developers
...
const string strMessageMagic = "Heirloom Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Heirloom: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // Heirloom: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Heirloom: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Heirloom: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // Heirloom: subsidy is cut in half every 16x multiply of difficulty
...
// Heirloom: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// Heirloom: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Heirloom: target change every block
    // Heirloom: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // Heirloom: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Heirloom: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // Heirloom: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // Heirloom: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Heirloom: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Heirloom: track money supply and mint amount info
    ...
    // Heirloom: fees are not collected by miners as in bitcoin
    // Heirloom: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// Heirloom: total coin age spent in transaction, in the unit of coin-days.
...
// Heirloom: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // Heirloom: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // Heirloom: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Heirloom: record proof-of-stake hash value
    ...
    // Heirloom: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // Heirloom: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // Heirloom: coinbase output should be empty if proof-of-stake block
    ...
    // Heirloom: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // Heirloom: check that the block satisfies synchronized checkpoint
    ...
    // Heirloom: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Heirloom: verify hash target and signature of coinstake tx
    ...
    // Heirloom: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Heirloom: getblocks may not obtain the ancestor block rejected
    ...
    // Heirloom: if responsible for sync-checkpoint send it
...
// Heirloom: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Heirloom", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Heirloom", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Heirloom: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // Heirloom: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // Heirloom: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // Heirloom: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // Heirloom: should not enter safe mode for longer invalid chain
    // Heirloom: if sync-checkpoint is too old do not enter safe mode
    ...
    // Heirloom: if detected invalid checkpoint enter safe mode
    ...
    // Heirloom: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // Heirloom: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Heirloom: record my external IP reported by peer
        ...
        // Heirloom: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // Heirloom: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Heirloom: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Heirloom: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Heirloom: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // Heirloom: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
