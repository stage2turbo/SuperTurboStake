...
// Heirloom: optional setting to unlock wallet for block minting only;
...
// Heirloom: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Heirloom: timestamp must not exceed spend time
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Heirloom: not avatar mode
                        {
...
// Heirloom: create coin stake transaction
...
// Heirloom: check 'spent' consistency between wallet and txindex
// Heirloom: fix wallet spent state according to txindex
...
// Heirloom: disable transaction (only for coinstake)
